...
const AudioActivity = ({ activity }) => {
  ...
  return (
    <div className="flex flex-col space-y-4">
      <audio ref={audioRef} src={`http://localhost:3001/${activity.audio}`} className="hidden" />
    ...
    <div className="flex flex-col space-y-6">
      ...
      >
        <div className="relative w-full h-full">
          ...
          >
            <h3 className="text-xl font-medium text-gray-800 text-center">{currentCard.front}</h3>
          </div>
      ...
      <div className="flex justify-between pt-4">
        ...
        <div className="flex space-x-2">
          ...
          >
            {completedCards.includes(currentCardIndex) ? 'Completed' : 'Mark as Complete'}
          </button>
...
const QuizActivity = ({ activity }) => {
  ...
  const [remainingAttempts, setRemainingAttempts] = useState(activity.completionCriteria?.attemptsAllowed || 3);
  ...
  const nextQuestion = () => {
    ...
    } else if (Object.keys(selectedAnswers).length === activity.questions.length) {
      setShowResults(true);
    }
  };
  ...
  const isPassed = () => {
    ...
    return scoreData.percentage >= (activity.completionCriteria?.passingScore || 70);
  };
        ...
        <div className="text-center">
          ...
          <div className="mb-4">
            <div className="inline-block p-4 rounded-full bg-indigo-100 mb-2">
              <span className="text-3xl font-bold text-indigo-600">{scoreData.percentage}%</span>
            </div>
          ...
          {isPassed() ? (
            <div className="bg-green-100 text-green-700 p-4 rounded-lg">
              <p className="font-medium">You passed! You can now continue to the next activity.</p>
            </div>
          ) : (
            <div className="bg-yellow-100 text-yellow-700 p-4 rounded-lg">
              <p className="font-medium">
                You need {activity.completionCriteria?.passingScore || 70}% to pass.
                {remainingAttempts > 0 ? ` You have ${remainingAttempts} attempts remaining.` : ' No more attempts remaining.'}
              </p>
            </div>
          )}
        </div>
  ...
  return (
    <div className="flex flex-col space-y-6">
      <div className="flex justify-between items-center text-sm text-gray-600">
        <span>Question {currentQuestionIndex + 1} of {activity.questions.length}</span>
          ...
          {currentQuestion.options.map((option, index) => (
            ...
            >
              <div className="flex items-center">
                <div className={`w-6 h-6 flex-shrink-0 rounded-full border ${
                  isAnswerSelected(option)
      ...
      <div className="flex justify-between pt-4">
        ...
        >
          {currentQuestionIndex === activity.questions.length - 1 && Object.keys(selectedAnswers).length === activity.questions.length ? 'Finish Quiz' : 'Next'}
        </button>
      </div>
    </div>
  );
